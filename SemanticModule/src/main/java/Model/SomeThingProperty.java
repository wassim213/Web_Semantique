 ...
 * @author CodeFox
 */
public class SomeThingProperty extends Property{

    public SomeThingProperty(String[] keysFr, String[] keysEn) {
        super(keysFr, keysEn);
    }
    ...
    public String getSparqlQuery(UserRequest request) {
        ...
        String[] ont = this.ontologyDefiner.getOntology(params[0]);
        if(ont == null) {
            ont = params;
        }
        return "SELECT ?result WHERE { \n" +
               "?complement a " + ont[1] + ".\n" +
               "?complement rdfs:label \"" + params[1] + "\"@" + request.getLanguage() + ".\n" +
               "?complement " + ont[0] + " ?entity.\n" +
